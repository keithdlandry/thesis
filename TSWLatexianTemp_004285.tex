...
\section{?history of matter/elementary physics/quarks?}
...
\caption[Interference Fragmentation Function]{an outgoing quark fragments into a $\pi^+\pi^-$ pair. This is described by the Interference Fragmentation Function (IFF). The Vector $\overrightarrow{R}$ gives the orientation of the pair.}
\label{fig:IFF}
\end{center}
\end{figure}
...
\begin{equation}
\label{eq:asymCS}
A_{UT} \sin (\phirs) =\frac{1}{|\overrightarrow{s_a}|} \frac{d\sigma^\uparrow - d\sigma^\downarrow}{d\sigma_{UU}}
\end{equation}
...
\begin{equation}
\label{eq:asymCS}
A_{UT} \sin (\phirs) =\frac{1}{|\overrightarrow{s_a}|} \frac{d\sigma^\uparrow - d\sigma^\downarrow}{d\sigma^\uparrow + d\sigma^\downarrow}
\end{equation}
...
\begin{figure}
\begin{center}
\includegraphics[width = 1\textwidth]{angleDeff_NEW}
\caption[Angles $\phis$ and $\phir$]{Visual representation of angles $\phi_R$ and $\phi_S$ }
\label{fig:angleDeff}
\end{center}
\end{figure}
...
\begin{equation}
\label{eq:mptrelate}
z = \frac{\overrightarrow{P_{\pip}} + \overrightarrow{P_{\pim}}}{\overrightarrow{P_q}}
\end{equation}
%
...
Just as we randomly assigned the spin state of the proton, we also randomly assign the charges on the $\pip,\pim$. This changes the direction of $\overrightarrow{R}$. 

\begin{equation}
\label{eq:sinphir}
\sin(\phi_R) = \frac{(P_B \times R)  \hat{P}_h}{|\hat{P}_h \times P_B||\hat{P}_h \times R|}
\end{equation}
\begin{equation}
\label{eq:cosphir}
\cos(\phi_R) = \frac{\hat{P}_h \times P_B}{|\hat{P}_h \times P_B|}  \frac{\hat{P}_h\times R}{|\hat{P}_h\times R|}
\end{equation}
%
By making the substitution $R\rightarrow -R$ in equations \ref{eq:sinphir} and \ref{eq:cosphir}, it is seen that $\phi_R \rightarrow \phi_R + \pi$ (or $\phi_R \rightarrow \phi_R - \pi$ because I keep $\phi_R$ in the range $-\pi$ to  $\pi$). Since $R$ doesn't appear in the definition of $\sin(\phi_S)$ or $\cos(\phi_S)$, $\phi_S$ is unchanged. This leads to $\phirs \rightarrow \phirs + \pi$ (or $\phirs \rightarrow \phirs - \pi$ because I constrain the angle to be in the range $-\pi$ to $\pi$). Just like the random spin assignment, the random pion charge assignment will be correct half the time and incorrect the other half. By looking at the cross ratio for each case we come to an equation similar to equation \ref{eq:ranSpin} with both factors canceling.
...
We know from the \pair pair case that the asymmetry should increase with the pseudorapidity of the pair. In figure \ref{fig:AutVMass_KPi} we had a lower $\etakp$ of 0. If we increased the lower limit, we should see an increase in the asymmetry. Seeing an increase in the asymmetry near the $K^*$ could be evidence that the asymmetry there is real and not a product of statistical fluctuations. The results can be seen in figure \ref{fig:AutVMass_KPi_eta050}. 
